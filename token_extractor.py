...
# Load environment variables for USER, PASS and SERVER from .env
...
class XiaomiCloudConnector:
    ...
    def login_step_2(self):
        ...
        if valid:
            ...
            valid = "ssecurity" in json_resp and len(
    ...
    def signed_nonce_sec(nonce, ssecurity):
        hash_object = hashlib.sha256(base64.b64decode(
            ssecurity) + base64.b64decode(nonce))
        return base64.b64encode(hash_object.digest()).decode('utf-8')
    ...
    def generate_enc_signature(url, method, signed_nonce, params):
        signature_params = [str(method).upper(), url.split("com")[
            1].replace("/app/", "/")]
        for k, v in params.items():
            signature_params.append(f"{k}={v}")
            ...
            for home in hh:
                ...
                if devices is not None:
                    if devices["result"]["device_info"] is None or len(devices["result"]["device_info"]) == 0:
                        print(
                            f'No devices found for server "{current_server}" @ home "{home["home_id"]}".')
                        continue
                    print(
                    ...
                    print(
                        f"Unable to get devices from server {current_server}.")
    else:
